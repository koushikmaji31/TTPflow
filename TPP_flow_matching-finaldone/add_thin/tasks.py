...
class Tasks(pl.LightningModule):
    ...
    def get_loss(self, log_prob_x_0,  x_n):
        """
    ...
    def step(self, batch, name):
        ...
        """
        # Forward pass
        log_prob_x_0, x_n, lossy = self.model.forward(batch)

        # Compute loss
        #loss,  intensity = self.get_loss( log_prob_x_0, x_n)

        # Log loss
        self.log(
            f"{name}/loss",
            log_prob_x_0.detach().item(),
...
class Forecasting(Tasks):
    ...
    def validation_step(self, batch, batch_idx):
        if self.global_step >= 1:
            ...
            print("val/MAE_counts: ", torch.cat(maes).mean())
            print("val/forecast_wasserstein_distance: ", wasserstein)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log(
            "test/forecast_wasserstein_distance",
            wasserstein,
            batch_size=batch.batch_size,
        )
...
class DensityEstimation(Tasks):
    ...
    def validation_step(self, batch, batch_idx):
        with torch.no_grad():
            if self.global_step >= 1:
                actual_lens = batch.mask.sum(-1)
                sample= self.model.sample(300, tmax=batch.tmax).to_time_list()
                ...
                self.log("val/sample_mmd", 100, batch_size=batch.batch_size)
                self.log(
                    "val/sample_count_wasserstein",
                    100 ,
                    batch_size=batch.batch_size,
            ...
            # print the mmd and wasserstein distance
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            self.log("test/sample_mmd", mmd, batch_size=batch.batch_size)
            self.log(
                "test/sample_count_wasserstein",
                wasserstein,
                batch_size=batch.batch_size,
            )
